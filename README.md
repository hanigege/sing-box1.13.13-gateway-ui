...
FakeIP 固定启用 UDP/443 保护：先对 FakeIP 网段的 QUIC 流量执行 `block`，同时对已被 FakeIP 还原成域名的 YouTube/Google 视频域名补一条 UDP/443 回落规则，让浏览器自动回落到 TCP/HTTPS，再继续按域名规则走直连或代理。这个兜底只收窄到 `googlevideo.com`、`youtube.com`、`youtube-nocookie.com`、`ytimg.com`、`ggpht.com` 和 `googleusercontent.com`，避免把全部国外 UDP/443 都拦掉。这样可以减少 YouTube、Google 等大流量 QUIC 长连接压住代理节点和连接表；真实目标 IP 的游戏 UDP、语音 UDP 和直连业务不受这条规则影响。这个保护属于网关稳定性边界，不在 UI 里提供关闭入口。
