...
# TProxy only captures FakeIP and explicit greylist IP/CIDR targets; proxy node servers stay on the normal route.
...
{proxy6_set}
  ...
  chain prerouting {{
    ...
    # 本机访问 FakeIP 或灰名单 IP/CIDR 时先在 output 打标，策略路由回 lo 后只处理已标记目标，避免误拦节点服务器 IP。
    ...
    # LAN 客户端只接管 FakeIP 和 UI 灰名单里明确配置的 IP/CIDR；其它真实公网 IP 不进入 TProxy。
  ...
  chain output {{
    ...
    # output 只给本机发往 FakeIP 和 UI 灰名单 IP/CIDR 的 TCP/UDP 打 TProxy mark，节点服务器 IP 保持系统原路由。
