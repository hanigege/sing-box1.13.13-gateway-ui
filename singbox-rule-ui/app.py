...
# Only FakeIP ranges are captured by TProxy; real IPv4/IPv6 destinations and proxy node servers stay on the normal route.
...
{bypass6}
  ...
  chain prerouting {{
    ...
    # 本机访问 FakeIP 时先在 output 打标，策略路由回 lo 后只在 lo 上交给 sing-box，避免误拦真实公网 IP 或节点服务器 IP。
    ...
    # LAN 客户端拿到 FakeIP 后仍由旁路网关接管；真实公网 IP 不再进入 TProxy，防止扩大拦截边界。
  ...
  chain output {{
    ...
    # output 只给本机发往 FakeIP 网段的 TCP/UDP 打 TProxy mark，真实公网 IP 和节点服务器 IP 保持系统原路由。
